...
class TxListAdapter : RecyclerView.Adapter<TxListAdapter.VH>() {

    private var items: List<TxOpUi> = emptyList()

    fun submit(list: List<TxOpUi>) {
        items = list
        notifyDataSetChanged()
    }

    fun getItemAt(position: Int): TxOpUi = items[position]
